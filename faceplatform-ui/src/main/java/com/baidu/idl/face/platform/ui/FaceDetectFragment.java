 ...
 * 人脸采集接口
 */
public class FaceDetectFragment extends RelativeLayout implements
    ...
    public static final String TAG = FaceDetectFragment.class.getSimpleName();
    ...
    public FaceDetectFragment(Context context) {
        super(context);

        if(inflate==null) {
            inflate = View.inflate(getActivity(), R.layout.activity_face_detect_v3100, null);
        }
        ...
        mRootView = inflate.findViewById(R.id.detect_root_layout);
...
//        });
        ...
        mViewBg = inflate.findViewById(R.id.view_bg);
        ...
        Log.i(TAG, "FaceDetectFragment: 111111");
    }
...
//    }
    ...
    protected void startPreview() {
        ...
        if (mIDetectStrategy != null) {
//            mIDetectStrategy.setPreviewDegree(90);
        }
...
//        mPreviewHight = point.y;
        // Preview 768,432
        mPreviewRect.set(0, 0, mDisplayWidth, mDisplayHeight);

//        mCameraParam.setPreviewSize(mDisplayWidth, mDisplayHeight);
//        mCamera.setParameters(mCameraParam);
    ...
    if(takePic) {
        try {
            YuvImage yuvimage = new YuvImage(data, ImageFormat.YUY2,1080, 1920, null);

            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            yuvimage.compressToJpeg(new Rect(0, 0, 1080,1920), 80, baos);

            String file="/mnt/sdcard/data" +  ".jpg";
            new File(file).delete();
            FileOutputStream outStream = new FileOutputStream(new File(file));
            outStream.write(baos.toByteArray());
            outStream.flush();
            outStream.close();
            if(onPhotoTakListener!=null){
//                onPhotoTakListener.onTakePic(file);
            }

        } catch (Exception e) {
            e.printStackTrace();
            Log.i(TAG, "onPreviewFrame: "+e.getMessage());
        }
...
//        Log.i(TAG, "onPreviewFrame: ");
        if (mIDetectStrategy == null ) {
            ...
            Log.i(TAG, "onPreviewFrame: --------------");
            Rect detectRect =new Rect(0,0,mDisplayWidth,mDisplayHeight);
            mIDetectStrategy.setDetectStrategyConfig(detectRect, detectRect, this);
        }
//        if (mIDetectStrategy != null) {
//            Log.i(TAG, "onPreviewFrame:detectStrategy ");
...
//        }
        ...
        if(listener!=null){
            listener.onDetectCompletion(status,message,base64ImageCropMap,base64ImageSrcMap);
        }
    ...
    public void setRecListener(RecListener listener) {
        ...
        mCamera.takePicture(new Camera.ShutterCallback() {
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                try {
                    ...
                    if(onPhotoTakListener!=null){
                        onPhotoTakListener.onTakePic(bitmap);
                    }
...
//        });
    ...
    OnPhotoTakListener onPhotoTakListener;




    public void setOnPhotoTakListener(OnPhotoTakListener onPhotoTakListener) {
        this.onPhotoTakListener = onPhotoTakListener;
    }

    public interface  OnPhotoTakListener{
        void onTakePic(Bitmap path);
