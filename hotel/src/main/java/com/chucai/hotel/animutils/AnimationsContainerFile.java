...
public class AnimationsContainerFile {
    private static final String TAG = "AnimationsContainerFile";
    public int FPS = 58;  // 每秒播放帧数，fps = 1/t，t-动画两帧时间间隔
    private Context mContext ;
    private File[] fileList;
    // 单例
    private static AnimationsContainerFile mInstance;


    public AnimationsContainerFile(Context context){
        this.mContext=context;
    }
    //获取单例
    public static AnimationsContainerFile getInstance(Context context,File[] resId, int fps) {
        if (mInstance == null)
            mInstance = new AnimationsContainerFile(context);
        mInstance.setResId(resId, fps);
        return mInstance;
    }

    public void setResId(File[] files, int fps){
        this.fileList=files;
        this.FPS = fps;
    }
//    // 从xml中读取资源ID数组
//    private int[] mProgressAnimFrames = getData(resId);
    ...
    public FramesSequenceAnimation createProgressDialogAnim(ImageView imageView) {
        return new FramesSequenceAnimation(imageView, fileList, FPS);
    }
    ...
    public class FramesSequenceAnimation {
        ...
        public FramesSequenceAnimation(ImageView imageView, File[] files, int fps) {
            ...
            if(files.length>0) {
                imageView.setImageBitmap(BitmapFactory.decodeFile(files[0].getAbsolutePath()));
                ...
                public void run() {
                    ...
                    if (imageView.isShown()) {
                        if (mBitmap != null) { // so Build.VERSION.SDK_INT >= 11
                            Bitmap bitmap = null;
                            try {
                                if(fileList[imageRes]!=null) {
                                    if(oneShot){
                                       if(imageRes!=fileList.length-1){
                                           bitmap = BitmapFactory.decodeFile(fileList[imageRes].getAbsolutePath(), mBitmapOptions);
                                       }else {
